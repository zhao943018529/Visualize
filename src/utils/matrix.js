 ...
 * @returns
 */
export function initialize(source, target) {
  if (source) {
    if (target) {
      for (var i = 0; i < 16; i++) {
        target[i] = source[i];
 ...
 * @returns
 */
export function multiply(next, prev, target) {
  target = target || new Float32Array(16);
  // 第一列
  var p00 = prev[0];
  var p10 = prev[1];
 ...
 * @returns
 */
export function translation(tx, ty, tz, target) {
  target = target || new Float32Array(16);
  for (var i = 0; i < 12; i++) {
    if (i % 5 == 0) {
      target[i] = 1;
 ...
 * @returns
 */
export function translate(m, tx, ty, tz, target) {
  target = target || new Float32Array(16);
 ...
 * @returns
 */
export function rotationX(angle, target) {
  target = target || new Float32Array(16);
 ...
 * @returns
 */
export function rotationY(angle, target) {
  target = target || new Float32Array(16);
  var sin = Math.sin(angle);
  var cos = Math.cos(angle);
 ...
 * @returns
 */
export function rotationZ(angle, target) {
  target = target || new Float32Array(16);
  var sin = Math.sin(angle);
  var cos = Math.cos(angle);
 ...
 * @returns
 */
export function axisRotation(axis, angle, target) {
  target = target || new Float32Array(16);
 ...
 * @returns
 */
export function lookAt(cameraPosition, lookTarget, upDirection, target) {
 ...
 * @returns
 */
export function perspective(viewRadians, aspect, near, far, target) {
  //投影盒上边坐标
 ...
 * @returns
 */
export function perspectiveOfRect(left, right, top, bottom, near, far, target) {
  target = target || new Float32Array(16);
 ...
 * @returns
 */
export function ortho(left, right, bottom, top, near, far, target) {
  target = target || new Float32Array(16);
...
export function makeRotationFromEuler(euler, target) {
  target = target || new Float32Array(16);
...
export function makeRotationFromQuaternion(q, target) {
  target = target || new Float32Array(16);
  var x = q.x,
    y = q.y,
    z = q.z,
...
export function scale(m, sx, sy, sz, target) {
  target = target || new Float32Array(16);
 ...
 * @returns
 */
export function scalation(sx, sy, sz, target) {
  target = target || new Float32Array(16);
  target[0] = sx;
  target[1] = 0;
  target[2] = 0;
...
export function rotateX(m, angleInRadians, target) {
...
export function cloneMatrix(source, target) {
  for (let i = 0; i < source.length; i++) {
    target[i] = source[i];
  }
  return target;
}
export function getMatrixFromEuler(euler, dst) {
