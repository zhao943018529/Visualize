...
function createCubeBufferInfo(
  ...
  const cubeOriginData = createCube(w, h, d);
  const { positions, colors, normals, texcoords } = transformIndicesToUnIndices(
    cubeOriginData
  ) as any;
  const cubeData = {
    color: colors,
...
export default function Robot() {
  ...
  useEffect(() => {
    ...
    const mainBufferInfo = createCubeBufferInfo(gl, 1, 4, 1);
    const topBufferInfo = createCubeBufferInfo(gl, 2, 1, 1);
    const bottomBufferInfo = createCubeBufferInfo(gl, 1, 1, 1);
    ...
    const blockGuyNodeDescriptions = {
      ...
      children: [
        ...
        },
        {
          name: "top",
          translation: [1, 1.5, 0],
          bufferInfo: topBufferInfo,
          children: [],
        },
        {
          name: "bottom",
          translation: [1, 0, 0],
          bufferInfo: bottomBufferInfo,
          children: [],
        },
      ],
    };
    ...
    function drawScene(time: number) {
      ...
      nodeInfosByName["Ferrari"].trs.rotation[0] = Math.abs(Math.sin(time * 3));
      nodeInfosByName["Ferrari"].trs.rotation[1] = Math.abs(Math.sin(time * 3));
